...
## Obtain Distances From Data Alone
...
init_path_river <- calculate_path_length(distances_river, PATH_RIVER)

TAU <- c(1000,500,100,50,25,20,10,5,2,1,.1,.01,.001,.0001)

#### FUNCTIONALIZE ####
#number of iterations proportional to temperature
k <- 1
path.dist <- c()
n.tau <- seq(100,10000, len=length(TAU))
for (i in 1:length(TAU)) {
  for (j in 1:n.tau[i]) {
    swap <- sample(1:5,2) #randomly select two cities to swap
    new.path <- PATH_RIVER
    #manually do the swap by switching the indices of the path and saving it as new path
    new.path[swap] <- PATH_RIVER[c(swap[2], swap[1])]  
    #calculate the path length of this new path
    iter_path <- calculate_path_length(distances_river, new.path)
    #this is the ratio p(y)/p(x)
    #if we happen to get a new path length that is smaller than the old path length, we save it
    if (runif(1) < exp((init_path_river - iter_path)/TAU[i])) { #code in something stochastic
      PATH_RIVER <- new.path
      init_path_river <- iter_path
    }
    path.dist[k] <- init_path_river
    k <- k + 1
  }
}
